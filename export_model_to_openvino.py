...
model_path, imgsz,dynamic, half, int8, batch_size = vars(args).values()

print(f"model: {model_path}, dynamic: {dynamic}, half: {half}, int8: {int8}, batch size: {batch_size}")
model = YOLO(model_path)
model.export(format="openvino",imgsz=imgsz,dynamic=dynamic, half=half, int8=int8, batch = batch_size)
